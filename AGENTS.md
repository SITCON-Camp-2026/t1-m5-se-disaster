...
v1 成果從 `/v1/` 存取。部署到 GitHub Pages 時，build 會複製 `dist/index.html` 到 `dist/v1/index.html`，避免靜態站台把 `/v1/` 當成不存在的實體目錄而 404。`?view=v1` 只作為相容入口保留。首頁 `/` 保留 Phase 0 入口，並提供進入行動者視角的明確入口。
...
維護 v1 流程前請先閱讀 `docs/flow.md`，再修改 `src/features/v1/**` 或視角切換行為。
...
`已確認任務` 只能在資料已被標示為 `verified` 時出現。`僅可前往核對` 只能代表現場幫手可以去核對資訊，例如公告、物資數量、集合點或報到規則；不得寫成正式派工、直接救災、直接送物資或可安全前往的保證。
